...
## Holds references to all Custom Logger subclasses and their enabled state
...
class LoggerRegistry(QtCore.QAbstractItemModel):
    def __init__(self):
        super().__init__()
        self._root = LoggerRegistry.Category(None, "")
    ...
    # (note that we left out 'CustomLoggers' - every play is assumed to be in a descendent module of it)
    def insert(self, module_path, logger_class):
        category = self.root

        # iterate up to the last one (the last one is just an underscored,
        # lowercased version of the logger's name and we don't display it in the tree)
        for module in module_path[:-1]:
            if not category.has_child_with_name(module):
                subcategory = LoggerRegistry.Category(category, module)
                category.append_child(subcategory)
            category = category[module]

        LoggerNode = LoggerRegistry.Node(module_path[-1], logger_class)
        # if playNode.module_name in category:
        #     raise AssertionError("There's already a play registered for the given module path")
        category.append_child(LoggerNode)

    def load_loggers(self, list_of_loggers):
        self.clear()
        for logger in list_of_loggers:
            node = self.node_for_module_path(logger)
            if node is not None:
                node.enabled = True
            else:
                logging.warn("Attempt to load non-existent logger " + '/'.join(
                    logger) + " from playbook.")

    def delete(self, module_path):
        node = self.node_for_module_path(module_path)
        del node.parent[node.name]

        # remove any categories where this logger was the only entry
        node = node.parent
    ...
    def clear(self):
        enabled_loggers = self.get_enabled_loggers_paths()
        for logger in enabled_loggers:
            node = self.node_for_module_path(logger)
            if node is not None:
                node.enabled = False
            else:
                logging.warn("Attempt to clear non-existent logger " + '/'.join(
                    logger) + " from custom logger registry.")
    ...
    ## Returns a list of module paths for the currently-enabled custom loggers
    # The module path is a list or tuple giving the path the the loggers' python module
    # For example: ['testing', 'test_pivot_kick']
    def get_enabled_loggers_paths(self):
        enabled_loggers = []
            ...
            if node.enabled:
                ...
                while curr_node is not None:
                    if curr_node.module_name:
                        logger_path.insert(0, curr_node.module_name)
                    curr_node = curr_node.parent

                enabled_loggers.append(logger_path)
    ...
    def __iter__(self):
        def _recursive_iter(category):
            for child in category.children:
                if isinstance(child, LoggerRegistry.Node):
                    yield child
                else:
    ...
    def __contains__(self, logger_class):
        for node in self:
            if node.logger_class == logger_class:
                return True
        return False
    ...
    class Node():
        def __init__(self, module_name, logger_class):
        ...
        @property
        def name(self):
            return self.logger_class.__name__
    ...
    def data(self, index, role):
        ...
        if role == QtCore.Qt.DisplayRole:
            if index.column() == 0:
                ...
                if isinstance(node, LoggerRegistry.Node):
                    return str(node.logger_class.score())
                else:
                    return None
        elif role == QtCore.Qt.CheckStateRole and isinstance(
                node, LoggerRegistry.Node):
            if index.column() == 0:
                return node.enabled
            else:
                return None
    ...
    def setData(self, index, value, role):
        if role == QtCore.Qt.CheckStateRole:
            if index.isValid():
                loggerNode = index.internalPointer()
                if not isinstance(loggerNode, LoggerRegistry.Node):
                    raise AssertionError(
                        "Only Logger Nodes should be checkable...")
                loggerNode.enabled = not loggerNode.enabled
